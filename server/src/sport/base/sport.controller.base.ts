...
export class SportControllerBase {
  ...
  async create(
    @common.Query() query: {},
    @common.Body() data: SportCreateInput,
    ...
    if (invalidAttributes.length) {
      ...
      throw new errors.ForbiddenException(
        `providing the properties: ${properties} on ${"Sport"} creation is forbidden for roles: ${roles}`
      );
    }
    // @ts-ignore
  ...
  async findMany(
    @common.Query() query: SportWhereInput,
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<Sport[]> {
    const permission = this.rolesBuilder.permission({
      role: userRoles,
  ...
  async update(
    ...
    if (invalidAttributes.length) {
      ...
      const roles = userRoles
        ...
        .join(",");
      throw new errors.ForbiddenException(
        `providing the properties: ${properties} on ${"Sport"} update is forbidden for roles: ${roles}`
      );
    }
    try {
      // @ts-ignore
  ...
  async createTeams(
    @common.Param() params: SportWhereUniqueInput,
    @common.Body() body: SportWhereUniqueInput[],
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<void> {
    const data = {
      teams: {
    ...
    if (invalidAttributes.length) {
      ...
      throw new common.ForbiddenException(
        `Updating the relationship: ${
          invalidAttributes[0]
        } of ${"Sport"} is forbidden for roles: ${roles}`
      );
    }
